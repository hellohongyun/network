...
### 3. 五机场阶梯式智能调度
同时接入 5 个机场，分成三个梯队，系统自动帮你**用对的梯队做对的事**：

```plain
看 YouTube → 主力梯队先上（速度快价格适中）
    ↓ 掉线了
    自动切换到保底梯队（最便宜）
    ↓ 也掉线了
    自动切换到优质梯队（最贵最稳）
...
每个梯队内有多个机场，自动跨机场选延迟最低的节点。
你选地区，系统选梯队和机场。**视频省钱，AI 稳定，全自动。**
...
每个主要地区背后是 **3 个梯队的测速组**，每个梯队内跨多机场选最快节点。你在面板上看到的是简洁的国旗，背后是精密的调度引擎。
...
### 第二步：上传到路由器
将 `configs/v7.yaml` 上传到 OpenWrt 的 Mihomo 插件配置目录，在插件页面选择该配置文件并启用。若 `rule-providers` 引用了本仓库的 `DIRECT-32.yaml`、`DIRECT-34-relays.yaml`、`DIRECT-34.yaml`，需将 **`configs/rulesets/` 一并推送到你 raw URL 对应的 Git 分支**（或改为指向你 fork 后的地址）。
