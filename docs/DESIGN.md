...
```
┌─────────────────────────────────────────────────┐
│ 第1层：机场子组（url-test，自动选最快节点）       │
│                                                  │
│  [C] 香港 = AirportC 的香港节点，自动选最快       │
│  [B] 香港 = AirportB 的香港节点，自动选最快       │
│  [A] 香港 = AirportA 的香港节点，自动选最快       │
├─────────────────────────────────────────────────┤
│ 第2层：地区组（fallback，按优先级故障转移）       │
│                                                  │
│  🇭🇰 香港   = fallback [C]→[B]→[A] （省流版）    │
│  🇭🇰 香港★  = fallback [A]→[B]→[C] （稳定版）   │
├─────────────────────────────────────────────────┤
...
|------|-------------|-------------|
| 故障转移顺序 | C→B→A | A→B→C |
| 设计意图 | 先用最便宜的机场 | 先用最好的机场 |
| 适用场景 | 视频/社交/日常 | AI/默认出口/全局代理 |
| 为什么这样设计 | 看视频流量大，优先省钱 | AI 长对话不能断，优先稳定 |

### 3.3 次要地区处理

英国/德国/法国/韩国节点数量少，不值得拆成 3 个机场子组。直接用 `url-test + include-all-providers + filter` 从全部机场选最快节点。
...
# 4. 在地区 fallback 组的 proxies 中插入 [D]
```
